document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- HOME CAROUSEL LOGIC ---
    const homeCarouselSlides = document.getElementById('home-carousel-slides');
    ...
    if (homeCarouselSlides) {
        let currentHomeSlide = 0;
        const totalHomeSlides = bestWorksData.length;

        // Inject slides
        bestWorksData.forEach((item, index) => {
            const slide = document.createElement('a');
            slide.href = `karya.html?category=${encodeURIComponent(item.category)}`;
            slide.className = 'home-slide';
            slide.innerHTML = `
                <img src="${item.image}" alt="${item.category}">
                <div class="home-category-name">${item.category}</div>
            `;
            homeCarouselSlides.appendChild(slide);

            const dot = document.createElement('div');
            dot.className = 'dot-home' + (index === 0 ? ' active' : '');
            dot.addEventListener('click', () => goToHomeSlide(index));
            homeCarouselDots.appendChild(dot);
        });

        const updateHomeCarousel = () => {
            homeCarouselSlides.style.transform = `translateX(-${currentHomeSlide * 100}%)`;
            const dots = homeCarouselDots.querySelectorAll('.dot-home');
            dots.forEach((dot, idx) => dot.classList.toggle('active', idx === currentHomeSlide));
        };

        const goToHomeSlide = (index) => {
        ...
        nextHome.addEventListener('click', (e) => { e.preventDefault(); nextHomeSlide(); resetAutoSlide(); });
        prevHome.addEventListener('click', (e) => { e.preventDefault(); prevHomeSlide(); resetAutoSlide(); });

        let autoSlideInterval = setInterval(nextHomeSlide, 5000);
        const resetAutoSlide = () => {
            clearInterval(autoSlideInterval);
            autoSlideInterval = setInterval(nextHomeSlide, 5000);
        };
    }
    ...
    if (galleryRoot) {
        galleryData.forEach((category, catIdx) => {
            ...
            section.id = `cat-${category.title.replace(/\s+/g, '-').toLowerCase()}`;

            section.innerHTML = `
                <div class="category-header">
                    <span class="category-toggle">
                        <svg viewBox="0 0 256 256"><path d="${caretDownPath}"></path></svg>
                    </span>
                    <h2 class="category-title">${category.title}</h2>
                </div>
                <div class="gallery-wrapper">
                    <div class="carousel-btn prev">
                        <svg viewBox="0 0 256 256"><path d="${caretLeftPath}"></path></svg>
                    </div>
                    <div class="gallery-container">
                        ${category.files.map((file, index) => {
                            const isVideo = file.toLowerCase().endsWith('.mp4');
                            const filePath = `${category.folder}/${encodeURIComponent(file)}`;
                            return `
                                <div class="gallery-item" data-cat="${catIdx}" data-idx="${index}" data-src="${category.folder}/${file}" data-type="${isVideo ? 'video' : 'image'}">
                                    ${isVideo ?
                    ...
                    <div class="carousel-btn next">
                        <svg viewBox="0 0 256 256"><path d="${caretRightPath}"></path></svg>
                    </div>
                </div>
            ...
            const pathEl = section.querySelector('.category-toggle path');
            header.addEventListener('click', () => {
                const isExpanded = section.classList.toggle('expanded');
                pathEl.setAttribute('d', isExpanded ? caretUpPath : caretDownPath);
            ...
            items.forEach(item => {
                item.addEventListener('click', (e) => {
                    e.stopPropagation();
                    currentCategoryIndex = parseInt(item.getAttribute('data-cat'));
                    currentItemIndex = parseInt(item.getAttribute('data-idx'));
                    openPreview();
                });
            });
        });
        ...
        if (targetCategory) {
            ...
            sections.forEach(section => {
                ...
                if (title.toLowerCase() === targetCategory.toLowerCase()) {
                    // Expand
                    section.classList.add('expanded');
                    section.querySelector('.category-toggle path').setAttribute('d', caretUpPath);
                    // Scroll to
                    setTimeout(() => {
        ...
        function openPreview() {
            const category = galleryData[currentCategoryIndex];
            const file = category.files[currentItemIndex];
            const isVideo = file.toLowerCase().endsWith('.mp4');
            const filePath = `${category.folder}/${encodeURIComponent(file)}`;

            modalMedia.innerHTML = `
                <div class="modal-nav">
                    <div class="carousel-btn prev-modal"><svg viewBox="0 0 256 256"><path d="${caretLeftPath}"></path></svg></div>
                    <div class="carousel-btn next-modal"><svg viewBox="0 0 256 256"><path d="${caretRightPath}"></path></svg></div>
                </div>
                <div class="modal-media-wrap">
                    ${isVideo ?
                        `<video src="${filePath}" controls autoplay loop></video>` :
            ...
            modalMedia.querySelector('.prev-modal').onclick = (e) => {
                e.stopPropagation();
                currentItemIndex = (currentItemIndex - 1 + category.files.length) % category.files.length;
                openPreview();
            };
            modalMedia.querySelector('.next-modal').onclick = (e) => {
                e.stopPropagation();
                currentItemIndex = (currentItemIndex + 1) % category.files.length;
                openPreview();
            };
        }
